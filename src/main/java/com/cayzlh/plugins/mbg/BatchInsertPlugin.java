...
import com.cayzlh.plugins.mbg.utils.PluginTools;
...
public class BatchInsertPlugin extends BasePlugin {
    ...
    public boolean validate(List<String> warnings) {
        ...
        if ("com.mysql.jdbc.Driver".equalsIgnoreCase(this.getContext().getJdbcConnectionConfiguration().getDriverClass()) == false
            ...
            warnings.add(":插件" + this.getClass().getTypeName() + "插件使用前提是数据库为MySQL或者SQLserver，因为返回主键使用了JDBC的getGenereatedKeys方法获取主键！");
            return false;
        }
    ...
    public boolean clientGenerated(Interface interfaze, TopLevelClass topLevelClass, IntrospectedTable introspectedTable) {
        ...
        logger.debug("(批量插入插件):" + interfaze.getType().getShortName() + "增加batchInsert方法。");
        ...
        logger.debug("(批量插入插件):" + interfaze.getType().getShortName() + "增加batchInsertSelective方法。");
    ...
    public boolean sqlMapDocumentGenerated(Document document, IntrospectedTable introspectedTable) {
        ...
        if (context.getPlugins().sqlMapInsertElementGenerated(element, introspectedTable)) {
            ...
            logger.debug("(批量插入插件):" + introspectedTable.getMyBatis3XmlMapperFileName() + "增加batchInsertSelective实现方法。");
        }
